...
module.exports = function createHandler(config) {
  ...
  const store = localRequire(config.storePath || '../lib/store');
  const { MODELS, lookupModel } = localRequire(config.modelsPath || '../lib/models');
  const adminHtml = localRequire(config.adminPagePath || '../lib/admin-page.js');
