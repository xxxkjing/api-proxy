...
module.exports = function createHandler(config) {
  ...
  let keyStates = API_KEYS.map(key => ({ key, coolUntil: 0, callCount: 0, disabled: false }));
   ...
   * - disabled（401 被剔除）的 key 永不参与选择
   */
  function pickKey({ preferLeastUsed = false } = {}) {
    const now = Date.now();
    let pool = keyStates.filter(k => !k.disabled && k.coolUntil <= now);
    if (!pool.length) pool = keyStates.filter(k => !k.disabled);
    ...
    // 全冷却时（pool === 所有未禁用 key）：选最早恢复的；否则从可用里随机
    const activeCount = keyStates.filter(k => !k.disabled).length;
    if (pool.length === activeCount && activeCount > 1 && keyStates.some(k => !k.disabled && k.coolUntil > now)) {
  ...
  // 首字优化：绝大多数 chunk 不含 usage，先用快速字符串检查跳过，避免每块都 JSON.parse
  ...
  async function* proxyStream(req, targetUrl, headers, body, startTs) {
    // 防截断核心：连接超时（首字节 60s 内必到）+ 客户端断开传播 + 总时长保护
    // 客户端可通过 req.abortSignal 中止；超时通过 AbortSignal.timeout 兜底
    const clientSignal = (req && req.abortSignal) || null;
    const ac = new AbortController();
    let timedOut = false;
    let clientAborted = false;

    const onClientAbort = () => { clientAborted = true; ac.abort(); };
    if (clientSignal) {
      if (clientSignal.aborted) clientAborted = true;
      else clientSignal.addEventListener('abort', onClientAbort, { once: true });
    }
    const timeoutMs = Number(process.env.STREAM_TIMEOUT_MS) || 600000; // 默认 10 分钟总上限
    ...
    if (status !== 429 && status < 400 && wantsStream && response.body) {
      ...
      try {
        ...
        // 上游中断（网络抖动/超时/连接重置/客户端取消）：不吞错，但补发 SSE [DONE] 标记，
        // 让客户端知道流正常结束，而不是收到半截数据后超时挂起
        const reason = timedOut ? 'timeout' : (clientAborted ? 'client_disconnect' : 'upstream_interrupt');
        console.error(`[proxyStream] stream interrupted (${reason}): ${e.message}`);
        if (!clientAborted) {
          // 客户端还在等 → 补 [DONE] 收尾，避免客户端挂到超时
          try { yield { type: 'chunk', chunk: 'data: [DONE]\n\n', streamInfo: { status, contentType, ttft, usage } }; } catch (_) {}
        }
      } finally {
  ...
  async function proxyRequest(req, resLike, apiKey) {
    ...
    // 归一化 model 名：QwenPaw 等客户端可能带 provider 前缀（如 "local/deepseek-v4-flash"），
    // 上游只认纯模型名。这里剥掉 "xxx/" 前缀，避免上游 404 model not found 引发 key 池崩溃。
    if (body && typeof body === 'string') {
      try {
        const parsed = JSON.parse(body);
        if (parsed && typeof parsed.model === 'string') {
          const slashIdx = parsed.model.indexOf('/');
          if (slashIdx > 0 && slashIdx < parsed.model.length - 1) {
            parsed.model = parsed.model.slice(slashIdx + 1);
            body = JSON.stringify(parsed);
          }
        }
      } catch (_) { /* 非 JSON body 原样转发 */ }
    }
  ...
  return async function (req) {
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        // 401：该 key 已失效 → 从号池剔除，换下一个 key 重试
        if (result.status === 401) {
          finalState.disabled = true;
          finalState.disabledBy = '401';
          finalState.coolUntil = 0;
          attempts += 1;
          console.warn(`Key ${finalState.key.slice(0, 8)}... -> 401, REMOVED from key pool (${attempts}/${maxAttempts})`);
          continue;
        }

        // 404 model not found：该 key 无此模型权限 → 从号池剔除（按模型维度），换下一个 key 重试
        if (result.status === 404 && result.body && result.body.includes('model is not found')) {
          finalState.disabled = true;
          finalState.disabledBy = '404';
          finalState.coolUntil = 0;
          attempts += 1;
          console.warn(`Key ${finalState.key.slice(0, 8)}... -> 404 model not found, REMOVED from key pool (${attempts}/${maxAttempts})`);
          continue;
        }
    ...
    // 全灭保护：若所有 key 都是因 404（模型不存在）被禁用（而非 401/429），
    // 说明是模型名/权限问题而非 key 失效 → 恢复所有 key，避免号池永久锁死。
    // （2026-09-22 根因：QwenPaw 带 provider 前缀的模型名 local/xxx 引发上游 404）
    if (keyStates.length > 0 && keyStates.every(k => k.disabled)) {
      const allDisabledBy404 = keyStates.every(k => k.disabled && k.disabledBy === '404');
      if (allDisabledBy404) {
        console.warn('[key-pool] all keys disabled by 404 (model not found) — resetting pool; check model name is valid');
        keyStates.forEach(k => { k.disabled = false; k.coolUntil = 0; });
      }
    }
