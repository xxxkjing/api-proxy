...
// 持久化：单文件覆盖写（lib/cloud-store.js 的 store.json 或本地 DATA_FILE），平替数据库
// 数据策略：热数据+统计+最近日志全部放 store.json（覆盖写），不产生追加型日志文件
...
// 当日日志：只进内存环形缓冲（MAX_LOGS 条），随 store.json 覆盖写持久化。
// 不再按天追加 logs/YYYY-MM-DD.json —— 那会导致 Blob 文件无限膨胀撑爆存储。
// 日志保留策略：最近 MAX_LOGS 条覆盖写，旧日志自然淘汰（平替数据库，覆盖旧内容）。
let _pendingLogs = [];
let _logTimer = null;
function queueArchiveLog(entry) {
  // 内存环形缓冲已由 recordRequest 维护；这里仅保留空实现避免调用方改动，
  // 云端持久化走 store.json 覆盖写（saveNow），不再产生追加型日志文件。
