...
// ---- 持久化（云端） ----
let _saveTimer = null;
let loaded = false;       // load 是否已完成（竞态保护）
let _pendingSave = null;  // load 完成前缓存的待写数据
...
function saveNow() {
  ...
  if (!loaded) {
    // load 未完成：先缓存，等 load 完成后再写（避免 load 覆盖内存后云端数据丢失）
...
async function load() {
  if (!cloud.isEnabled()) { loaded = true; return; }
  try {
    const payload = await cloud.readJson("store.json");
    if (payload) {
      ...
      console.log(`[store] cloud merged ${clientKeys.size} keys, ${minuteBuckets.size} buckets, ${requestLogs.length} logs`);
    }
  } catch (e) {
    console.error("[store] cloud load failed:", e.message);
  } finally {
    loaded = true;
    // load 完成后，把待保存的数据写回云端
    if (_pendingSave) {
      const p = _pendingSave;
      _pendingSave = null;
      cloud.writeJson("store.json", p).catch(e => {
        console.error("[store] cloud pending save failed:", e.message);
      });
    }
  }
}
