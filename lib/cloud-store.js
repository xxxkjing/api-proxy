...
async function readJson(pathname) {
  ...
  if (!sdk) return null;
  try {
    const blob = await sdk.get(pathname, { type: "json", useCache: false });
    if (!blob || blob.statusCode === 404) return null;
    return blob;
  } catch (e) {
    if (e.statusCode === 404) return null; // 不存在是正常情况
...
async function writeJson(pathname, obj) {
  ...
  try {
    const blob = await sdk.put(pathname, JSON.stringify(merged), {
      access: "private",
      allowOverwrite: true,
